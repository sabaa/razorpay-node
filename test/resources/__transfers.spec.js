...
module.exports.test9_0 = function () {
{
    ...
    rzpInstance.transfers.create(params).then(response => {
        assert.equal(response.__JUST_FOR_TESTS__.url, '/v1/transfers', 'Create Transfer request url formed');
...
module.exports.test9_1 = function () {
{
    ...
    rzpInstance.payments.transfer(TEST_PAYMENT_ID, params).then(response => {
        assert.equal(response.__JUST_FOR_TESTS__.url, `/v1/payments/${ TEST_PAYMENT_ID }/transfers`, 'Create payment transfer request url formed');
        assert.ok(equal(response.__JUST_FOR_TESTS__.requestBody, expectedParams), 'All params are passed in request body');
        done();
    });
}}
...
module.exports.test9_2 = function () {
{
    ...
    rzpInstance.transfers.edit(TEST_TRANSFER_ID, params).then(response => {
        assert.equal(response.__JUST_FOR_TESTS__.url, `/v1/transfers/${ TEST_TRANSFER_ID }`, 'Edit transfer request url formed');
        assert.ok(equal(response.__JUST_FOR_TESTS__.requestBody, expectedParams), 'All params are passed in request body');
        done();
    });
}}
...
module.exports.test9_3 = function () {
{
    ...
    rzpInstance.transfers.reverse(TEST_TRANSFER_ID, params).then(response => {
        assert.equal(response.__JUST_FOR_TESTS__.url, `/v1/transfers/${ TEST_TRANSFER_ID }/reversals`, 'Edit transfer request url formed');
        assert.ok(equal(response.__JUST_FOR_TESTS__.requestBody, expectedParams), 'All params are passed in request body');
        done();
    });
}}
...
module.exports.test9_4 = function () {
{
    ...
    rzpInstance.transfers.fetch(TEST_TRANSFER_ID).then(response => {
        assert.equal(response.__JUST_FOR_TESTS__.url, `/v1/transfers/${ TEST_TRANSFER_ID }`, 'Fetch transfer url formed correctly');
        done();
    });
}}
...
module.exports.test9_6 = function () {
{
    ...
    rzpInstance.transfers.all({
        ...
        assert.equal(response.__JUST_FOR_TESTS__.url, `/v1/transfers?from=${ fromDateInSecs }&to=${ toDateInSecs }&count=25&skip=5`, 'Params are appended as part of request');
        done();
    });
}}
